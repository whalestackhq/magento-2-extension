...
class Create extends Action
{
    ...
    public function execute()
    {
        ...
        $client = new Api\WsMerchantClient($this->apiKey, $this->apiSecret);
        ...
        {
            Api\WsLoggingService::write($response->responseBody, 'Whalestack customer could not be created');
            $this->logger->critical('Whalestack customer could not be created', ['exception' => $response->responseBody]);
        ...
        $displayMethod = $this->scopeConfig->getValue('payment/whalestack_paymentgateway/price_display_method', ScopeInterface::SCOPE_STORE);
        $settlementAsset = $this->scopeConfig->getValue('payment/whalestack_paymentgateway/settlement_currency', ScopeInterface::SCOPE_STORE);
        $checkoutLanguage = $this->scopeConfig->getValue('payment/whalestack_paymentgateway/checkout_language', ScopeInterface::SCOPE_STORE);
        ...
        if ($displayMethod == 'simple') {
             ...
             * If Magento grand order total does not match Wss charge total, use simple checkout object
            ...
            if ($response->httpStatusCode != 200)
            {
                Api\WsLoggingService::write($response->responseBody, 'Whalestack checkout charge could not be validated.');
                $this->logger->critical('Whalestack checkout charge could not be validated', ['exception' => $response->responseBody]);
                throw new LocalizedException(__($response->responseBody));
            }
        ...
        $checkout['webhook'] = $this->urlBuilder->getUrl('whalestack/payment/webhook');
        $checkout['pageSettings']['cancelUrl'] = $this->urlBuilder->getUrl('whalestack/payment/cancel', ['order_id' => $order->getId()]);
        $checkout['pageSettings']['returnUrl'] = $this->urlBuilder->getUrl('whalestack/payment/success');
        ...
        if ($response->httpStatusCode != 200)
        {
            Api\WsLoggingService::write($response->responseBody, 'Whalestack checkout failed');
            $this->logger->critical('Whalestack checkout failed', ['exception' => $response->responseBody]);
            throw new LocalizedException(__($response->responseBody));
        }
        $data = json_decode($response->responseBody, true);

        /**
         * Update order with Whalestack Checkout Id
         */

        $order->setWhalestackCheckoutId($data['id']);
        $order->save();
