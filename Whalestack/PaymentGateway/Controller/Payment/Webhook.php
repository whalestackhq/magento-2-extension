...
class Webhook extends Action
{
    ...
    public function execute()
    {
        try {
            ...
            /**
             * Find Magento order by Whalestack checkout id
             */
        ...
        switch ($payload['eventType']) {
            ...
            case 'CHECKOUT_COMPLETED':
                ...
                if (!in_array($order->getStatus(), array('complete', 'processing'))) {
                    ...
                    $paymentDetailsPage = 'https://www.whalestack.com/en/payment/checkout-id/' . $checkout['id'];

                    $order->addStatusHistoryComment(__('Order PAID via Whalestack. See payment details <a href="' .  $paymentDetailsPage. '" target="_blank">here</a>.'));
            ...
            case 'CHECKOUT_UNDERPAID':
                ...
                if (!in_array($order->getStatus(), array('complete', 'processing'))) {
                    $checkout = $payload['data']['checkout'];
                    $paymentDetailsPage = 'https://www.whalestack.com/en/unresolved-charge/checkout-id/' . $checkout['id'];

                    $order->addStatusHistoryComment(__('Whalestack payment was underpaid by customer. See details and options to resolve it <a href="' .  $paymentDetailsPage. '" target="_blank">here</a>.'));
            ...
            case 'REFUND_COMPLETED':
                ...
                if (in_array($context, array('COMPLETED_CHECKOUT', 'UNDERPAID_CHECKOUT'))) {
                    ...
                    $paymentDetailsPage = 'https://www.whalestack.com/en/refund/' . $refund['id'];
            ...
            default:
                ...
                Api\WsLoggingService::write($payload, 'Unresolved payload event for order id ' . $order->getId());
    ...
    private function validate_webhook($request_headers, $payload)
    {
        ...
        $api_secret = $this->scopeConfig->getValue('payment/whalestack_paymentgateway/api_secret');
    ...
    /**
     * Get order by Whalestack checkout id
     */
    private function getOrder($checkoutId)
    {
        $searchCriteria = $this->searchCriteriaBuilder
            ->addFilter('whalestack_checkout_id', $checkoutId, 'eq')->create();
